<template>
  <v-toolbar color="indigo" dark fixed app>
   ...
   <span v-if="this.$store.state.isUserLoggedIn">
      ...
      ></v-progress-circular>
      <v-avatar :title="this.$store.state.user.fullName" v-if="!this.$store.state.loading.userInfo">
        <img
...
const configJSON = require(`../../../config/${process.env.NODE_ENV === 'production' ? 'production' : 'default'}.json`)
export default {
  data: () => (
    {
